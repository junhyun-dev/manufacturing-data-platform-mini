...
이 프로젝트는 manufacturing-style/tabular 작업 데이터를 CSV 또는 bounded Kafka landing으로 받아서, 나중에 다시 설명할 수 있도록 **입력, 처리 결과, 품질검사, lineage, version metadata**를 남기는 작은 데이터 플랫폼이다.
...
|---|---|---|---|---|
| Source input | raw manufacturing-style rows/events | CSV 또는 Kafka record | source contract | 회사/고객 실제 데이터 포함 금지 |
...
최근 Kafka 문서가 많지만 Kafka가 시스템 전체는 아니다. 현재 구현을 한 장으로 보면 아래와 같다.

```text
CSV ------------------------------------------+
                                               |
Kafka -> durable raw landing -> batch adapter -+-> 기존 batch pipeline
                                                    -> bronze / silver / gold
...
Airflow -> 위 pipeline/publish CLI를 호출
operator report -> 남은 evidence를 읽음
```

핵심은 `기존 batch spine`이다. Kafka는 입력을 durable하게 받는 방법을 추가했고,
Spark/Iceberg는 정정 결과를 table/snapshot으로 publish하는 방법을 추가했다.
...
local lakehouse gold -> Iceberg partition publish
local Airflow `dags test` + standalone/LocalExecutor wrappers
local Kafka K1 bounded raw landing and failure/replay evidence
Kafka K1.5 landing -> deterministic batch adapter -> existing pipeline
```
...
continuous Kafka streaming / Spark Structured Streaming
multi-partition Kafka rebalance and ordering
ROS2/MCAP ingest
...
## 8. Spark/Iceberg는 왜 추가됐나

Spark/Iceberg는 같은 `business_date` 정정 결과를 단순 파일 append가 아니라
partition 교체와 snapshot evidence로 표현하기 위해 추가했다.
...
```text
single local gold table
business_date partition overwrite
other partition preserved
pipeline run_id -> Iceberg snapshot_id evidence
```
...
전체 bronze/silver/gold를 Spark로 옮긴 것은 아니다. 먼저 기본 질문을 Python/CSV에서
고정한 뒤, 정정 publish 질문 하나만 Spark/Iceberg로 다시 표현했다.
...
```text
1. 이 plain map을 읽는다.
2. 01-system-traceability-map.ko.md에서 시나리오별 연결을 본다.
3. 관심 있는 scenario 하나를 고른다.
4. 그 scenario가 가져온 question-bank 영역만 읽는다.
5. 연결된 decision/slice/test를 본다.
6. 최신 실행 결과는 VERIFICATION_LOG.md에서 확인한다.
```

먼저 이 문장을 말할 수 있으면 전체 구조를 잡은 것이다.

> 이 프로젝트는 manufacturing-style synthetic CSV와 bounded Kafka landing을 기존 batch spine으로 연결해 bronze/silver/gold 지표를 만들고, 그 결과가 어느 입력에서 왔는지, 품질검사를 통과했는지, 재실행해도 중복되지 않는지를 기록하는 작은 데이터 플랫폼입니다.
