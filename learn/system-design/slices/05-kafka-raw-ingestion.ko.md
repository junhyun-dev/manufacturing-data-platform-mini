...
> Kafka Test 0 runbook과 client pin은 추가되었고 local broker round-trip은 검증됐다.
> K1 producer/raw-landing consumer와 restart/replay evidence는 아직 없다.
...
K1 구현 전 working direction이며 아직 Accepted decision이 아니다.
...
K1 raw landing code: none
K1 restart/replay tests: none
```
...
이 검증은 환경과 최소 client/broker 계약만 닫는다. raw landing, crash-window dedup,
restart, replay는 아래 K1 test contract가 구현될 때까지 claim하지 않는다.
