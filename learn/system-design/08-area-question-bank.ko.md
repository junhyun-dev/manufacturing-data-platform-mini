...
| 항목 | 현재 답 | 상태 |
|---|---|---|
| local SparkSession | PySpark 3.5.8 + Iceberg jar로 실행됨 | 검증됨 |
...
| 항목 | 현재 답 | 상태 |
|---|---|---|
| successful run reuse | 구현됨 |
| quality fail result | 일부 구현됨 |
| failed partial-state forensics | 구현하지 않음 |
...
| Airflow runtime trigger | Airflow 미설치 | 미검증 |
| Airflow-triggered Spark runtime | 구현하지 않음 | Backlog |
...
| path-level lineage | 구현됨 |
| column-level lineage | 구현하지 않음 |
| metrics/alerts | 구현하지 않음 |
...
|---|---|---|
| synthetic data | 명시됨 |
| public secret scan | publication checklist로 수행 |
| PII governance | 구현하지 않음 |
| retention policy | 구현하지 않음 |
...
| 항목 | 현재 답 | 상태 |
|---|---|---|
| toy local input | Python 충분 |
| Spark scale proof | 구현하지 않음 |
| cost/retention | Backlog |
...
|---|---|---|
| pytest | 40 passed |
| Spark optional tests | PySpark 설치 후 2 passed |
| CLI smoke | lakehouse/EAV/operator/Spark CLI 검증 |
| version pin | `07-spark-iceberg-version-pin.md` |
...
| Airflow | wrapper command contract test-covered | runtime trigger verified |
...
기존 `02-slice2-question-map.md`와 `06-spark-iceberg-walking-skeleton-plan.md`의 질문은 아래처럼 읽으면 된다.
