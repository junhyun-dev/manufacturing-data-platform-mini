...
| downstream DAG는 시간 스케줄로 도나, gold asset 업데이트로 trigger되나? | "매일 2시"로 돌릴지, "위 데이터가 갱신되면" 돌릴지 정한다. | cross-pipeline 의존을 시간이 아니라 데이터 준비 상태로 표현한다. Airflow 3.x 기준으로는 Assets 용어를 쓴다. | time schedule only(현재) / Airflow Asset update trigger / Asset Watcher | Backlog(named): 현재는 단일 DAG manual trigger. asset-based scheduling은 구현하지 않는다. |
...
| 어떤 executor를 쓰고, 긴 대기 task는 deferrable로 둘 것인가? | task를 실제로 돌리는 주체와, 기다리는 동안 worker 자리를 놓아줄지 정한다. | executor 선택이 worker dependency packaging의 크기를 정한다. triggerer는 deferrable operator가 worker slot 없이 대기하게 한다. | LocalExecutor(현재) / CeleryExecutor / KubernetesExecutor; sync BashOperator / deferrable operator | Backlog(named): 현재 local standalone은 LocalExecutor. distributed executor와 deferrable은 구현하지 않는다. |
...
| lineage를 bespoke JSON이 아니라 OpenLineage 포맷으로 emit할 것인가? | 계보를 특정 도구가 읽을 수 있는 표준 형식으로 내보낼지 정한다. | Airflow OpenLineage provider와 Spark listener 같은 표준 emission 경로가 있다. 현재 path-level JSON은 정직하지만 고립적이다. | bespoke path-level JSON(현재) / OpenLineage emit to Marquez or DataHub / column-level lineage | Backlog(named): OpenLineage backend/통합은 구현하지 않는다. 표준 emit은 다음 단계 reference로만 명시한다. |
| gold의 소비자-facing output contract가 있는가? | 입력 약속뿐 아니라 gold가 소비자에게 보증하는 컬럼/타입/not-null 약속이 있는가? | source contract는 입력 방어이고 output contract는 소비자 신뢰다. gold 변경 시 downstream impact/exposure와 직결된다. | 없음(현재) / gold schema 문서화 / dbt-style model contract 강제 | Backlog(named): 현재는 gold grain 문서화까지다. runtime contract 강제는 구현하지 않는다. |
