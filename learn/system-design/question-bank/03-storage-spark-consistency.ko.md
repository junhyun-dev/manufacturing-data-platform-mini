...
|---|---|---|---|---|
| 어떤 catalog를 쓸 것인가? | table 이름과 metadata를 누가 관리하는가? | local hadoop catalog와 production REST/Hive/Glue catalog는 운영 성격이 다르다. | hadoop / Hive / REST / Glue / Nessie | Core-lite: local hadoop catalog. production catalog는 Backlog. |
| Iceberg current snapshot이 source of truth인가? | downstream은 어떤 snapshot을 읽는가? | run folder와 table current가 동시에 있으면 기준이 필요하다. | latest run folder / JSON current pointer / Iceberg current snapshot | publish slice: Iceberg current table은 publish 결과, JSON catalog가 publish source. |
...
| quality fail run이 current table로 갈 수 있는 경로가 있는가? | 실패한 데이터가 downstream에 노출되는가? | quality gate와 publish gate의 연결이 핵심이다. | no publish on fail / publish with warning / quarantine table | publish CLI는 latest successful JSON state만 읽는다. |
