...
이 노트는 Slice2에서 "table에 write를 어떻게 commit하나"를 다룬다. `slices/spark-iceberg-partition-overwrite/01-question-map.md`의 #1(ACID) + #2(write semantics) + #10(idempotency)을 하나로 수렴한다. 이게 Slice2의 심장이다.

관련 문서/코드:

- [`../system-design/slices/spark-iceberg-partition-overwrite/02-state-shift.md`](../system-design/slices/spark-iceberg-partition-overwrite/02-state-shift.md) (§3.1 run_id≠snapshot_id, §3.2 layer 모델)
- [`../system-design/slices/spark-iceberg-partition-overwrite/01-question-map.md`](../system-design/slices/spark-iceberg-partition-overwrite/01-question-map.md)
