...
| 복구 완결 gate 발행 (S9) | local bounded 조합 구현·runtime 검증·accepted-closed | `tests/test_recovered_telemetry_publish.py`, `scripts/verify_recovered_telemetry_publish.sh`, `dags/manufacturing_recovered_telemetry_publish.py`, publish evidence JSON | S8 readiness gate를 재구현하지 않고 공유, 봉인 event 집합과 adapter 입력의 정확한 동등성 검사, 기존 S7 quality gate와 Iceberg `business_date` overwrite 그대로 재사용; 미완결 복구는 Spark/Iceberg state를 남기지 않고, 같은 입력 재실행은 새 snapshot·partition overwrite를 만들지 않음(S7이 skip 판정 전에 Spark/quality를 돌리므로 전체 no-op은 아님). evidence는 이번 시도의 `spark_attempt_run_id`와 재사용된 snapshot을 구분해 기록; streaming sink·cluster Spark·concurrent writer·production Airflow claim 아님 |
...
```text
production Airflow deployment는 out of scope
...
```text
failure-state forensics 또는 portfolio promotion
-> 검증된 Kafka -> batch -> quality/gold/Iceberg evidence 재사용
